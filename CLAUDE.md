...
**No cross-recipe references — and when you do mention another recipe, crosslink it both ways.** Never write "smaller than the X above", "as in the previous recipe", "see the meatballs we just covered", or any phrasing that assumes the reader is reading recipes in the order they were drafted. Each recipe is published as a standalone page; what's "above" in a drafting session does not exist in the reader's view. The validator flags suspicious phrasings like "the X meatballs above", "the X version above", "as in the section above"; treat these as bugs.

When you legitimately need to mention another recipe in prose (regional sibling, format comparison, technique parent), do **two** things:

1. **Inline the actual recipe title** so the auto-linker fires. The link map keys on each entry's `title`, case-insensitive, so writing "the 40 to 50 g format of Italian-American pork meatballs in tomato" rather than "the Italian-American meatball format" gives you a working `x-link` in the step text without any explicit markup.
2. **Add a `related_recipes` frontmatter entry** so the structural "Related recipes" block at the bottom of the page lists it with a one-line `why`. Shape: `related_recipes: [{ recipe_slug: 'recipes/<slug>', why: '<one sentence>' }]`. The validator hard-errors on missing-or-malformed `recipe_slug` and self-references; `check.mjs` errors on a slug that doesn't resolve to a page.

`related_recipes` is different from `homemade_alternatives` (store-bought-to-homemade upgrade path) and from algorithmic "Related" (tag-derived). Use it for *authored* sibling crosslinks: regional variants, format comparisons, alternative-technique paths. Use it sparingly — two or three at most per recipe; long lists dilute the signal.
