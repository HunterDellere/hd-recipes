---
type: 'ingredient'
category: 'ingredients'
status: 'complete'
content_review: 'pending'

title: 'Wood ear mushroom (木耳)'
desc: 'Dehydrated Auricularia heimuer fungus — black, gelatinous when rehydrated, and prized for texture rather than flavor. The crunch is what it does.'
metaDesc: 'Wood ear mushroom: dried black fungus that rehydrates 5x. How to soak it, what it adds to dishes, and the food-safety rule that matters.'
tags: ['chinese', 'vegetable']
density_g_per_ml: 0.1
updated: '2026-04-26'

usda_fdc_id: 169403

about: |
  Wood ear is sold dried as small, brittle, black-brown chips. Rehydrated in cool water for 30–60 minutes it expands roughly 5x by weight, going from leather-crisp to a translucent, jellied sheet with a distinct crunch that survives cooking. The crunch is structural — wood ear is rich in chitin-glucan polymers and a polysaccharide gel matrix that holds its shape against heat far longer than any cap mushroom does.

  Flavor-wise it is nearly inert. The job is texture and visual contrast. In hot-and-sour soup it provides the dark ribbons that catch on the spoon; in mu shu pork and yu xiang dishes it gives the dish something to bite against the soft pork and slick sauce. Treat it like a vegetable that will contribute crunch and adsorb seasoning, not like a flavor mushroom.

  One food-safety note worth taking seriously: rehydrate in cool water for under two hours, or in the refrigerator for longer soaks. Wood ear left rehydrating at room temperature for 12+ hours has caused cases of bongkrekic acid poisoning from Burkholderia gladioli contamination. The toxin is heat-stable. Soak short and cold.

  At the store, look for whole pieces rather than shredded: whole gives you control over the size and the shredded product is often older stock. Two main varieties: 木耳 (heimuer, the standard) and 银耳 (silver/snow ear, Tremella fuciformis), which is white and used in sweet desserts. They are not interchangeable.

storage: 'Dried, airtight, dry pantry. 1+ year. Once rehydrated, refrigerate and use within 2 days; do not re-dry rehydrated pieces.'

substitutions:
  - for: 'wood ear mushroom'
    use: 'no good substitute'
    note: 'Nothing else rehydrates to that crunch-against-jelly texture. Fresh shiitake or king oyster gives flavor but not the structural bite. If unavailable, leave the dish without rather than fake it.'
---
