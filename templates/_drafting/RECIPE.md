...
3. **Ingredient selection — always the best form, not the default form.** Before listing any ingredient, ask which form produces the best outcome in this specific recipe. Common cases where the right choice is non-obvious:
    - **Vanilla in visible or lightly cooked applications** (pancakes, waffles, custard, ice cream, frosting, sugar cookies, panna cotta): use `vanilla bean paste` — you get the visible seeds and a more concentrated aromatic load. Use `vanilla extract` only where vanilla is a background note in a heavily flavored bake (brownies, chocolate cake) or where heat will drive off the minor volatiles anyway.
    - **Butter in any baked good or sauce where browning is an option**: ask whether brown butter earns its complexity here. If yes, call it explicitly in the steps and technique reference.
    - **Salt**: `kosher salt` (Diamond Crystal) for brining, dry-brining, pasta water, and general seasoning. `fine sea salt` where it dissolves into batter or dough (baking, emulsions) — coarser grains don't incorporate evenly. Never leave salt unspecified.
    - **Sugar**: `granulated sugar` for structure and Maillard browning; `light brown sugar` when molasses depth is part of the flavor profile; `dark brown sugar` when assertive molasses is the point (gingerbread, baked beans, certain glazes). Never default to "sugar" without specifying which.
    - **Fat for finishing vs. cooking**: clarified butter or a neutral oil for high-heat cooking; whole butter only where its milk solids are a feature (browning, basting, emulsifying).
    Read `content/ingredients/<slug>.md` for any ingredient that has an obvious variant before finalizing the ingredient list. The `substitutions` section of the ingredient page will tell you which form is correct for which use case.
