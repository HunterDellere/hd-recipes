...
(function () {
  ...
  // ingredient's `seasonality:` field. Hidden by default; revealed only when
  // the build emitted at least 2 matching recipes.
  async function initInSeason(entries) {
    ...
    let data;
    try {
      data = await loadJson('data/in-season.json');
    } catch {
      return;
    }
