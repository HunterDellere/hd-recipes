...
(function () {
  ...
  // ── Personal cook log ─────────────────────────────────────────────────
  // Tracks how many times this user has cooked this recipe, plus the date
  // of the last cook. Stored locally only — no analytics, no telemetry.
  // Storage key shape: hdr-cooked:<pathname> → { count, last: ISO date }
  const logBtn = document.querySelector('[data-cook-log]');
  const logBadge = document.querySelector('[data-cook-log-badge]');
  if (logBtn && logBadge) initCookLog(logBtn, logBadge);

  function initCookLog(btn, badge) {
    const key = `hdr-cooked:${location.pathname}`;
    function read() {
      try {
    ...
    function fmtDate(iso) {
      // Short form like "May 9" — locale-aware, year omitted unless it's not the current year.
      try {
    ...
    function paint() {
      ...
      if (!v || v.count <= 0) {
        ...
        btn.querySelector('.rh-log-label').textContent = 'I cooked this';
        return;
      }
      badge.hidden = false;
      const times = v.count === 1 ? '1×' : `${v.count}×`;
      const dateStr = v.last ? ` · last ${fmtDate(v.last)}` : '';
      badge.textContent = `Cooked ${times}${dateStr}`;
      btn.classList.add('is-logged');
      btn.querySelector('.rh-log-label').textContent = 'Cooked again';
    }
    btn.addEventListener('click', () => {
      const v = read() || { count: 0, last: null };
      v.count += 1;
      v.last = new Date().toISOString();
      write(v);
      paint();
      // Tiny celebration — accent color flash on the button
      btn.classList.add('rh-log-pulse');
      setTimeout(() => btn.classList.remove('rh-log-pulse'), 500);
    });
    // Long-press to undo (touch) / right-click to undo (desktop)
    btn.addEventListener('contextmenu', (e) => {
      const v = read();
      if (!v || v.count <= 0) return;
      e.preventDefault();
      if (!confirm('Undo last cook log entry?')) return;
      v.count -= 1;
      if (v.count <= 0) {
